...
void testExecuteSQL()
{
	...
	// "select a.FID ,Code_Khet,Khet_Rom,Khet_unico from addressed_roadMap_V6.93_Points_Province as a,addressed_roadMap_V6.93_Points_Updated_Commune as b where a.FID = b.FID "
	//OGRLayer* layer = data.m_pGdalDataset->ExecuteSQL("select * from poi and hydro where NameEn = 'Agribank'", nullptr, nullptr);

	//ReadData data("D:\\Data\\02_vietmap-2017-06-release-20170823");
	//// "select a.FID ,Code_Khet,Khet_Rom,Khet_unico from addressed_roadMap_V6.93_Points_Province as a,addressed_roadMap_V6.93_Points_Updated_Commune as b where a.FID = b.FID "
	OGRLayer* layer = data.m_pGdalDataset->ExecuteSQL("select * from like admin-point left join poi on admin-point.Id=poi.Id", nullptr, nullptr);
...
{
	if (!m_pGdalDataset)	return nullptr;
	else  return m_pGdalDataset->GetLayerByName(pLayerName);
}
...
OGRFeature* ReadData::getFeature(OGRLayer* pLayer, int id)
{
	...
	else  return pLayer->GetFeature(id);
}
...
{
	if (!pLayer)  return OGRwkbGeometryType(-1);//zh+,accept:if(type==null) else?
	else  return pLayer->GetGeomType();
}
...
{
	if (!pLayer) return -1;
	else return pLayer->FindFieldIndex(pName, bExactMatch);
}
...
OGRFieldType ReadData::getFieldType(OGRFeature *pFeature, int fieldIndex)
{
	...
	if (!pFieldDefn)  return OGRFieldType(-1);
	else  return pFieldDefn->GetType();
}
...
const char* ReadData::getDatasetName()
{
	...
	else return m_pGdalDataset->GetDescription();
}
